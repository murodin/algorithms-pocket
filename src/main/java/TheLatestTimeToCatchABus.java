...
public class TheLatestTimeToCatchABus {
    ...
    public static void main(String[] args) {
        System.out.println("Solution: " + Solution.latestTimeCatchTheBus(new int[]{20,30,10}, new int[]{19,13,26,4,25,11,21}, 2));
    }

    // Time: O(MN), where M is the number of the buses, N is the number of the passengers.
    // Space: O(1)
    static class Solution {
        public static int latestTimeCatchTheBus(int[] buses, int[] passengers, int capacity) {
            ...
            int n= buses.length, m= passengers.length, j= 0, count= 0;
            for (int bus : buses)
                for (count = 0; j < m && count < capacity && passengers[j] <= bus; j++)
                    count++;
