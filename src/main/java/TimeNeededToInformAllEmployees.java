...
public class TimeNeededToInformAllEmployees {
    ...
    public static void main(String[] args) {
        System.out.println("Solution: " + Solution.numOfMinutes(6, 2, new int[] {2,2,-1,2,2,2}, new int[] {0,0,1,0,0,0}));
    }
   ...
   static class Solution {
        public static int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
            ...
            for (var i = 0; i < n; i++)
                adjList.computeIfAbsent(manager[i], k -> new ArrayList<Integer>())
                        .add(i);

            return findTime(adjList, informTime, headID);
        }

        private static int findTime(Map<Integer, List<Integer>> adjList, int[] informTime, int employee) {
            var minutes = 0;

            for (var report : adjList.getOrDefault(employee, List.of()))
                minutes = Math.max(minutes, findTime(adjList, informTime, report));

            return informTime[employee] + minutes;
        }
    }
}
