...
public class MinimumInitialEnergyToFinishTasks {
    ...
    public static void main(String[] args) {
        System.out.println("Solution: " + Solution.minimumEffort(new int[][]{{1,2},{2,4},{4,8}}));
    }

    // Time: O(NLogN)
    // Space: O(1)
    static class Solution {
        public static int minimumEffort(int[][] A) {
            int res = 0;
            Arrays.sort(A, Comparator.comparingInt(a -> (a[1] - a[0])));
            for (int[] a : A) {
